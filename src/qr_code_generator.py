...
class QRCodeGenerator:
    ...
    def generate(self, qr_id, size=300):
        ...
        if self.config['border_width'] > 0:
            bordered_qr = Image.new('RGBA', (size + self.config['border_width'] * 2, 
                                            size + self.config['border_width'] * 2), 
                                   self.config['border_color'])
            bordered_qr.paste(qr_img, (self.config['border_width'], self.config['border_width']), qr_img)
            qr_img = bordered_qr
    ...
    def _add_icon_with_frame(self, qr_img, qr_size):
        ...
        qr_center = qr_size // 2
        frame_width = self.config.get('frame_width', 8) if self.config.get('frame_enabled', True) else 0
        frame_inner_padding = self.config.get('frame_inner_padding', 4)
        total_cutout_size = icon_size + (frame_inner_padding * 2) + (frame_width * 2) + self.config['cutout_padding'] * 2
        ...
        if self.config['cutout_shape'] == 'rounded_square':
            ...
            cutout_draw.rounded_rectangle([cutout_x, cutout_y, cutout_x + total_cutout_size, cutout_y + total_cutout_size], 
                                         radius=cutout_radius, fill=0)

        if qr_img.mode != 'RGBA':
            qr_img = qr_img.convert('RGBA')
        
        qr_alpha = qr_img.split()[-1]
        qr_alpha = Image.composite(qr_alpha, Image.new('L', qr_img.size, 0), cutout_mask)
        ...
        if self.config['cutout_shape'] == 'rounded_square':
            cutout_radius = total_cutout_size // 6
            cutout_bg_draw.rounded_rectangle([cutout_x, cutout_y, cutout_x + total_cutout_size, cutout_y + total_cutout_size], 
                                            radius=cutout_radius, fill=self.config['cutout_background'])
        ...
        if self.config.get('frame_enabled', True) and frame_width > 0:
            ...
            if self.config['cutout_shape'] == 'rounded_square':
                ...
                cutout_bg_draw.rounded_rectangle([frame_outer_x, frame_outer_y, 
                                                frame_outer_x + frame_outer_size, frame_outer_y + frame_outer_size], 
                                               radius=frame_radius, fill=frame_color)
                
                inner_radius = frame_inner_size // 8
                cutout_bg_draw.rounded_rectangle([frame_inner_x, frame_inner_y, 
                                                frame_inner_x + frame_inner_size, frame_inner_y + frame_inner_size], 
                                               radius=inner_radius, fill=self.config['cutout_background'])
